...
export class RegistrationDetailsComponent implements OnInit {
  ...
  passwordPattern: any =/^(?:(?=.*[a-z])(?:(?=.*[A-Z])(?=.*[\d\W])|(?=.*\W)(?=.*\d))|(?=.*\W)(?=.*[A-Z])(?=.*\d)).{8,}$/;
  ...
  constructor(
    ...
    this.maxDate = new Date(this.maxDate.setFullYear(this.maxDate.getFullYear() - 25));
  }
  ...
  changePasswordBtn() {
    this.changePasswordMode = this.changePasswordMode === "text" ? "password" : "text";
  }
